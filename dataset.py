...
class LibraryThings(torch.utils.data.Dataset):
    def __init__(self, mode='train'):
        ...
        self.userIdx['<unk>'] = self.num_users
        self.itemIdx['<unk>'] = self.num_items
...
class LibraryThingsTest(torch.utils.data.Dataset):
    def __init__(self, train_data):
        self.userIdx = train_data.userIdx
        self.itemIdx = train_data.itemIdx
    ...
    def __getitem__(self, idx):
        user, item, rating = self.users[idx], self.items[idx], self.ratings[idx]
        if user not in self.userIdx.keys():
            user = '<unk>'
        if item not in self.itemIdx.keys():
            item = '<unk>'
        user, item = self.userIdx[user], self.itemIdx[item]
        return user, item, rating
